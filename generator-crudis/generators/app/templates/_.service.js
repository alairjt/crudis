(function () {
    ...
    app.factory('<%= capitalize(crudName) %>Service', ['<%= capitalize(crudName) %>', function (<%= capitalize(crudName) %>) {
        ...
        <%= capitalize(crudName) %>Service.buscar = function (filtros, cbSuccess, cbError) {
            return <%= capitalize(crudName) %>.query({id: id}, cbSuccess, cbError);
        };

        <%= capitalize(crudName) %>Service.salvar = function (<%= crudName %>, id, cbSuccess, cbError) {
            if (<%= capitalize(crudName) %>Service.emEdicao(id)) {
        <%= capitalize(crudName) %>.update(<%= crudName %>, {id: id}, cbSuccess, cbError);
            } else {
        <%= capitalize(crudName) %>.save(<%= crudName %>, {id: id}, cbSuccess, cbError);
            }
        };
